...
| `--hook` / `--no-hook` | off | Cold-open: prepend a ~4s teaser before the chronological cut, with the hook card on top saying what's coming. Without `--detect-payoff` it's the video's strongest beat (biased toward late peaks); with it, the teaser is drawn from the **tail of the demo span** so it shows the finished result — never the code/terminal that built it. Opt-in on the CLI — add `--hook` to enable (the Telegram bot defaults it ON). |
| `--detect-payoff` | off | Have Claude find the demo/payoff span — the stretch where the built thing's **output is on screen** (the rendered result, not the code editor or terminal). It plays at strict 1.0x however hard the rest compresses, the cold open teases its finished result (drawn from the span's tail, so no code/terminal leaks in), and its line words the hook card (needs the `claude` CLI; best-effort). The bot runs this automatically at upload. |
