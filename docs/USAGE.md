...
| `--caption-pos` | `auto` | `auto` builds a saliency map (motion + detail + brightness over the whole video) and places the caption over the calmest region in the **top half** of the TikTok safe zone — it favors the top so it stays clear of the main action (which usually sits lower), only dropping down within that band when the very top is busy. `top` pins it just below the top UI bar; `bottom` uses a letterboxed band below the video (legacy style — risks TikTok UI overlap). |
