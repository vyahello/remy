...
```bash
sudo nano /etc/tokcut/env        # template: deploy/env.example
```

| Variable | Where it comes from |
|---|---|
| `TELEGRAM_BOT_TOKEN` | @BotFather (same bot you use now) |
| `TOKCUT_ALLOWED_USER_ID` | your numeric id (@userinfobot) |
...
```bash
sudo systemctl start tokcut-botapi tokcut-bot
journalctl -u tokcut-bot -f      # expect: api=local Bot API (…, ≤2 GB)
```
...
```bash
journalctl -u tokcut-bot -f                  # live logs
systemctl restart tokcut-bot                 # manual restart
docker logs tokcut-bot-api --tail 50         # Bot API server logs
du -sh /home/tokcut/.tokcut/work             # workdir size (self-cleans)
```
...
- **Claude token expiry**: `claude setup-token` tokens are long-lived
  ...
  re-run `claude setup-token` locally and update `/etc/tokcut/env`.
